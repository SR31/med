...
>('user/login', async ({ email, password }, { rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(e.response?.data?.message ?? 'Ошибка соединения с сервером');
  }
});

export const register = createAsyncThunk<
  void,
  { email: string; password: string; full_name: string; role: 'PATIENT' | 'DOCTOR' },
  { rejectValue: string }
>('user/register', async (dto, { dispatch, rejectWithValue }) => {
  try {
    ...
    return rejectWithValue(e.response?.data?.message ?? 'Не удалось зарегистрироваться');
  }
});
